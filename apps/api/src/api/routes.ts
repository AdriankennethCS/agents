...
import { NicheLLMModelAgent } from '../agents/model/NicheLLMModelAgent';
...
export function createRoutes(
  ...
  // Arbitrage specific routes
  router.use('/decisions', createDecisionsRouter());
  router.use('/replay', createReplayRouter());
   ...
   * Get decision logs for an agent with pagination
   */
  router.get('/agents/:id/decisions', (req: Request, res: Response) => {
    try {
      const params = AgentIdParamSchema.parse({ id: req.params.id });
      ...
      const decisionsResult = logStore.queryDecisions(params.id, undefined, undefined, {
      ...
      res.json({
        ...
        pagination: {
          limit: decisionsResult.limit,
          offset: decisionsResult.offset,
          total: decisionsResult.total,
          hasMore: decisionsResult.hasMore,
        },
      });
    } catch (error) {
      if (error instanceof z.ZodError) {
      ...
      console.error('[API] Error in GET /agents/:id/decisions:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
   ...
   * Get calibration summary for a model-driven agent
   */
  router.get('/agents/:id/calibration', (req: Request, res: Response) => {
    try {
      const params = AgentIdParamSchema.parse({ id: req.params.id });
      const query = CalibrationQuerySchema.parse(req.query);
      ...
      // Get agent instance
      const instance = AgentRegistry.getInstance(params.id);
      if (!instance || !(instance instanceof NicheLLMModelAgent)) {
        return res.status(400).json({ 
          error: `Agent ${params.id} is not a model-driven agent or has not been initialized yet.` 
        });
      }

      const summary = instance.getCalibrationTracker().computeScores(query.periodDays);
      res.json(summary);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: 'Invalid request', details: error.errors });
      }
      console.error('[API] Error in GET /agents/:id/calibration:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
