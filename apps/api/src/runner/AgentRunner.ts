...
export class AgentRunner {
  ...
  private async processAgent(agentId: string, markets: MarketSnapshot[]): Promise<void> {
    ...
    // Create or get strategy from registry
    let strategy;
    try {
      strategy = AgentRegistry.getOrCreateStrategy(
        agentId,
        state.strategyType as any,
        strategyConfig
      );
    } catch (error) {
    ...
    // Get decision trace from strategy
    let trace;
    try {
      trace = strategy.decide({ state, markets });
    } catch (error) {
