...
export function seedAgents(stateStore: IStateStore): void {
  ...
  // Agent v1: V1 Deterministic Strategy (Paper Mode)
  const agentV1Id = 'agent-v1';
  const agentV1State: AgentState = {
    agentId: agentV1Id,
    name: 'V1 Deterministic Strategy',
    strategyType: 'v1',
    bankroll: 1000,
    startBankroll: 1000,
    pnlTotal: 0,
  ...
  stateStore.setAgentState(agentV1Id, agentV1State);
  // Note: V1 strategy uses StrategySpec, not StrategyConfig
  // StrategySpec is managed separately in V1AgentRunner

  console.log(`[seedAgents] Seeded 4 agents: ${agent1Id}, ${agent2Id}, ${agent3Id}, ${agentV1Id}`);
}
